...
public class GachaBanner {
    ...
    // These don't change between banner types (apart from Standard having three
    // extra 4star avatars)
    @Getter
    ...
    @Getter
    private int[] fallbackItems4Pool1 = DEFAULT_FALLBACK_ITEMS_4_POOL_1;
    @Getter
    private int[] fallbackItems4Pool2 = DEFAULT_FALLBACK_ITEMS_4_POOL_2;
    // Different banner types have different defaults, see above for default values
    // and the enum for
    ...
    @Getter
    private boolean autoStripRateUpFromFallback = true; // Ensures that featured items won't "double dip" into the
                                                        // losing pool

    private int[][] poolBalanceWeights4 = {
            { 1, 255 }, { 17, 255 }, { 21, 10455 }
    }; // Used to ensure that players won't go too many rolls without getting something
       // from pool 1
    ...
    // Deprecated fields that were tolerated in early May 2022 but have apparently
    // still being
    // circulating in new custom configs
    // For now, throw up big scary errors on load telling people that they will be
    // banned outright in
    // a future version
    @Deprecated
    ...
    public GachaInfo toProto(Player player) {
        // TODO: use other Nonce/key insteadof session key to ensure the overall
        ...
        String record =
                "http"
                        ...
                        + "&scheduleId="
                        + scheduleId;
        String inBanner = // Currently this is Record
                "http"
                        + (HTTP_ENCRYPTION.useInRouting ? "s" : "")
                        + "://"
                        + lr(HTTP_INFO.accessAddress, HTTP_INFO.bindAddress)
        ...
        GachaInfo.Builder info = GachaInfo.newBuilder()
                ...
                .setGachaProbUrl(inBanner)
                .setGachaProbUrlOversea(inBanner)
